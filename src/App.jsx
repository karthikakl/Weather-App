...
import {getWindDirection,getHumidityValue,getVisibilityValue,convertTemperature} from '../src/components/Helper'
...
const App = () => {
 ...
 const API_KEY = '469577cca9602c726cdba18c4a3330a9'
  ...
  //fetches 5 locations suggestion from api and update 
const fetchSuggestions = async(query)=>{
  ...
  //prevents form submission validates city and fetches data vis api key
  ...
  // Check weather condition and return an object
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-6 z-10">
        <div className="bg-white/10 backdrop-blur-md rounded-xl shadow-2xl p-8 max-w-md w-full text-white border border-white/30">
         ...
         {!weather ? (
          ...
          <input 
            ...
            className='mb-4 w-64 p-3 rounded border border-white bg-transparent text-white placeholder-white 
                       focus:outline-none focus:border-blue-300 transition duration-300 text-center'
          />
          
          {suggestion.length > 0 && (
            <div className='absolute top-20 left-1/2 transform -translate-x-1/2 bg-white/10 backdrop-blur-sm shadow-md rounded z-10 w-64'>
              {suggestion.map((s) => (
                <button 
                  type='button' 
                  key={`${s.lat}-${s.lon}`}
                  ...
                  className='block hover:bg-blue-700 bg-transparent px-4 py-2 text-sm text-left w-full text-white transition-colors'
                >
                  {s.name}, {s.country}{s.state && `, ${s.state}`}
                </button>
              ))}
          ...
          <button 
            type='submit' 
            className='bg-purple-700 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded transition-colors'
          >
            Get weather
          </button>
        </form>
